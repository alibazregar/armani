...
interface IOtherOrderModel extends Model<IOtherOrder> {
  calculatePrice(type: string): Promise<number>;
}

const OtherOrderSchema = new Schema<IOtherOrder, IOtherOrderModel>({
...
OtherOrderSchema.methods.calculatePrice = async function (
  number: number
): Promise<number> {
...
const OtherOrder = model<IOtherOrder, IOtherOrderModel>(
  "OtherOrder",
  OtherOrderSchema
);
